...
let dbConnFactory = 
    new NpgsqlConnection("Host=localhost;Username=dev;Password=dev;Database=myapp")
    
let queryHandler = fun (next: HttpFunc) (ctx: HttpContext) ->
                        task {
                        let! qp = ctx.BindModelAsync<QueryPlan>()
                        let! result = QueryProcessor.execute qp dbConnFactory
